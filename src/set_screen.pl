...
#
#indx#	set_screen.pl - Specify URL for screen to display
#@HDR@	$Id$
#@HDR@
...
########################################################################
#doc#	set_screen.pl - Specify URL for screen to display
########################################################################
...
use cpi_vars;
use cpi_file qw( fatal read_file read_lines write_file echodo files_in
  cleanup );
...
my $LOGTAG = $$;
my $LOGFILE = "/var/log/stderr/$cpi_vars::PROG";
if( $ENV{SCRIPT_FILENAME} )
    {
    close( STDERR );
    open( STDERR, ">> $LOGFILE" ) || die("Cannot append stderr:  $!");
    }

my $LIBDIR		=	"$cpi_vars::BASEDIR/lib";
my $SCREENS_LIST	=	"$LIBDIR/screens.list";
my $URLS_LIST		=	"$LIBDIR/urls.list";
my $URLS_SCRIPT		=	"$LIBDIR/URLs.html";
my $MESSAGES_SCRIPT	=	"$LIBDIR/messages.html";

my $WWWDIR = "/home/chris/public_html/set_screen";
...
my $URL_DIR		=	"$cpi_vars::BASEDIR/URLs";
my $MESSAGE_DIR		=	"$cpi_vars::BASEDIR/messages";
...
our %ONLY_ONE_DEFAULTS =
    (
    "alignment"	=>	"",		# Alignment tl tc tr ml mc mr bl bc br
    ...
    "function"	=>	"popup",	# window, frame, popup
...
#########################################################################
#	Return a list of structs containing frequently used URLs	#
#########################################################################
sub known_urls
    {
    my @ret;
    foreach my $ln ( &read_lines($URLS_LIST) )
	{
	push( @ret, {name=>$1,url=>$2} )
	    if( $ln =~ /^\s*(.*?)\s*(http.*?)\s*$/ );
	}
    return @ret;
    }
...
#########################################################################
#	Read in all the aliases for known screen names.			#
#	Each screen must be listed at least once in the screen map.	#
#########################################################################
sub read_screen_map
    {
    foreach my $ln ( &read_lines($SCREENS_LIST) )
	{
	my( $name, @aliases ) = split(/\s+/,$ln);
	$SCREEN_MAP{lc($name)} = $name;
	foreach my $alias ( @aliases )
	    { $SCREEN_MAP{lc($alias)} = $name; }
	}
    }
...
sub ip_to_host
    ...
    if( $localname && $localname =~ /^localhost/ )
	{
...
if( $ENV{SCRIPT_NAME} )
    ...
    #$hdrcount++;	# This would avoid Content-type:  text/html
