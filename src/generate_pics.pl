...
#
#indx#	generate_pics.pl - Generate set_screen commands to make wall of pics
#@HDR@	$Id$
#@HDR@
...
########################################################################
#doc#	generate_pics.pl - Generate set_screen commands to make wall of pics
#doc#	Not even close to finished code, but useful when you want to get
#doc#	a demo up quickly or are fracking around.  Became "generate_screens".
########################################################################
...
my $BASEDIR = "/u0/non_sys/usr/local/projects/set_screen";
...
sub generate_commands
	...
	if( $file =~ m:([^/]*)\.(\w+)$: )
	    {
	    my( $id, $ext ) = ( $1, $2 );
	    next if( ! grep( $_ eq $ext, split(/,/,$ARGS{u}) ) );
	    my $text = $id;
	    $text =~ s/_/ /g;
	    push( @cmds, join(" ",
		"set_screen",
